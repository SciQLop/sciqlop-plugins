...
def test_plot_line(preview):
    x, y = _make_line_data()
    preview.plot_variable(values=y, epochs=x, labels=["test"])
...
def test_plot_spectrogram(preview):
    ...
    preview.plot_variable(
        values=z, epochs=x, depend_1=y,
        labels=["spec"], display_type="spectrogram",
    )
...
def test_switch_line_to_spectrogram(preview):
    x, y = _make_line_data()
    preview.plot_variable(values=y, epochs=x, labels=["line"])
    assert not preview._is_colormap

    x2, y2, z2 = _make_spectrogram_data()
    preview.plot_variable(
        values=z2, epochs=x2, depend_1=y2,
        labels=["spec"], display_type="spectrogram",
    )
    assert preview._is_colormap
    assert preview._cmap_graph is not None
...
def test_switch_spectrogram_to_line(preview):
    ...
    preview.plot_variable(values=y2, epochs=x2, labels=["line"])
    assert not preview._is_colormap
    assert preview._line_graph is not None
...
def test_multiple_switches(preview):
    ...
    for i in range(5):
        x, y = _make_line_data()
        preview.plot_variable(values=y, epochs=x, labels=[f"line{i}"])
        assert not preview._is_colormap

        x2, y2, z2 = _make_spectrogram_data()
        preview.plot_variable(
            values=z2, epochs=x2, depend_1=y2,
            labels=[f"spec{i}"], display_type="spectrogram",
        )
        assert preview._is_colormap
...
def test_labels_updated_on_reuse(preview):
    ...
    x, y = _make_line_data()
    preview.plot_variable(values=y, epochs=x, labels=["first"])
    assert preview._line_graph.labels() == ["first"]

    x2, y2 = _make_line_data()
    preview.plot_variable(values=y2, epochs=x2, labels=["second"])
...
def test_multicomponent_labels(preview):
    ...
    preview.plot_variable(values=y, epochs=x, labels=["Bx", "By", "Bz"])
...
def test_component_count_change(preview):
    ...
    preview.plot_variable(values=y2, epochs=x2, labels=["Bt"])
...
def test_component_count_increase(preview):
    ...
    preview.plot_variable(values=y, epochs=x, labels=["Bt"])

    x2, y2 = _make_line_data(components=3)
    preview.plot_variable(values=y2, epochs=x2, labels=["Bx", "By", "Bz"])
    assert preview._line_graph.line_count() == 3
...
def test_label_count_mismatch_fewer(preview):
    ...
    x, y = _make_line_data(components=3)
    preview.plot_variable(values=y, epochs=x, labels=["only_one"])
    assert preview._line_graph is not None
    assert preview._line_graph.line_count() == 3
...
def test_label_count_mismatch_more(preview):
    ...
    x, y = _make_line_data(components=1)
    preview.plot_variable(values=y, epochs=x, labels=["a", "b", "c"])
    assert preview._line_graph is not None
    assert preview._line_graph.line_count() == 1
