"""Kimi Code session listing and transcript replay, over ACP.

Sessions live in the Kimi Code (Node) CLI's own store and are reachable
through the ACP session/list and session/load methods. Both are async and
need a live `kimi acp` process, while the SciQLop backend protocol is sync
and called on the GUI thread — so each operation spawns a short-lived agent
...
async def _spawn_agent():
    """A fresh `kimi acp` process with an initialized ACP connection."""
    import acp
...
def list_sessions() -> List[SessionEntry]:
    async def _list():
        proc, conn = await _spawn_agent()
        try:
            entries = []
            cursor = None
            while True:
                resp = await conn.list_sessions(
                    cwd=str(current_workspace_dir()), cursor=cursor,
                )
...
def load_session_messages(session_id: str, image_tempdir: Path) -> List[ChatMessage]:
    ...
    try:
        ...
        return []
    return _replay_to_messages(collected, Path(image_tempdir))


def _replay_to_messages(updates: list, image_tempdir: Path) -> List[ChatMessage]:
    from acp.schema import (
        AgentMessageChunk,
        AgentThoughtChunk,
        ToolCallProgress,
    ...
    from .backend import _raw_input_dict, _tool_output
