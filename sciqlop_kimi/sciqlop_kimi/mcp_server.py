"""In-process MCP server exposing the SciQLop tools to the Kimi Code agent.

Kimi Code runs as a separate `kimi acp` process, so the tools can't be plain
in-process classes the way the legacy Python SDK allowed. ACP's session/new
accepts HTTP MCP servers, so we serve the SciQLop tool surface over
streamable HTTP on a loopback port, from a uvicorn task sharing SciQLop's
qasync event loop — handlers keep their Qt main-thread affinity and no
subprocess IPC is needed.
...
class SciqlopMcpServer:
    def __init__(
        ...
        self._server: Server = Server("sciqlop")
