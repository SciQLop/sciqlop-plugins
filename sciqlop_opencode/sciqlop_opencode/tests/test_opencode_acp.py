...
def test_backend_is_acp_shell_with_opencode_command():
    acp_core = pytest.importorskip("SciQLop.components.agents.acp")
    from sciqlop_opencode.backend import OpencodeBackend

    assert issubclass(OpencodeBackend, acp_core.AcpAgentBackend)
    assert OpencodeBackend.display_name == "Opencode"
    assert OpencodeBackend.supports_sessions is True
...
def test_acp_command_is_not_shared_mutable_state():
...
def test_fetch_models_falls_back_to_default_when_the_cli_is_missing(monkeypatch):
    ...
    monkeypatch.setattr(bk.shutil, "which", lambda _: None)
    monkeypatch.setattr(bk, "acp_config_options", lambda _cmd: pytest.fail(
        "must not spawn an agent when the CLI is absent"))
...
def test_module_exposes_no_sdk_era_helpers():
    ...
    for name in ("_OpencodeStream", "_wrap_tool", "_reorder_required_first",
                 "_normalize_schema_types", "SYSTEM_PROMPT",
