...
- **`test_settings.py` × 5 failures** — pre-existing on `main`. Pydantic-bound-clamp regressions in `settings.py`'s `RadioSettings`. Not introduced by this branch; do NOT include in any catalog-feature fix.
